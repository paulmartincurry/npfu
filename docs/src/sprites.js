// NEW: Tiny animator + draw helpers.
// If no spritesheet is available, we render the old rectangles so the game still works.
...
export class Animator {
  ...
  draw(c, x, y, w, h, face=1){
    if(!this.s?.img){ return false; }
    const f = this.frame;
    const sx = (f * this.s.w), sy = 0;
    c.save();
    if(face === -1){ c.translate(x+w, y); c.scale(-1,1); x = 0; y = 0; }
...
export function drawActor(c, actor, rebel=false){
  ...
  // Fallback rectangles (previous style)
